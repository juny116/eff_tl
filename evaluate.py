...
def main():
    ...
    if args.task_name is not None:
        ...
        raise NotImplementedError('ONLY IMPLEMENTED FOR GLUE BENCHMARK.')
    ...
    assert "test" in processed_datasets.keys(), f'Test set not in dataset, got :  {processed_datasets.keys()}'
    ...
    with open(wrong_answer_file_path, "w") as file_writer:
        ...
        for step, batch in enumerate(test_dataloader):
            with torch.no_grad():
                ...
                for batch_index in range(args.per_device_batch_size):
        ...
        if args.local_rank == 0:
            writer.add_scalar('Test/Accuracy', eval_metric['accuracy'], 0)
            if "f1" in eval_metric.keys():
                writer.add_scalar('Test/F1', eval_metric['f1'], 0)
